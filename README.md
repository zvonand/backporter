...
Or use a venv: `python3 -m venv .venv && .venv/bin/pip install -e .`
...
```bash
backporter -t owner/repo:branch -p https://github.com/owner/repo/pull/123
```

### Example: fork backport with changelog description

```bash
backporter -C ../releases/248 -p https://github.com/ClickHouse/ClickHouse/pull/72226 --make-description -t Altinity/ClickHouse:customizations/24.8.14
```
...
- Python 3.10+
- Git
- [PyGithub](https://github.com/PyGithub/PyGithub) (installed automatically with `pip install -e .`)
...
```bash
backporter -C /path/to/repo -t myorg/myrepo:main -p https://github.com/myorg/myrepo/pull/42
```
...
```bash
backporter --conflicts-resolved -C ../releases/248 -t Altinity/ClickHouse:customizations/24.8.14
```
...
```bash
backporter --make-description -p https://github.com/owner/repo/pull/123
```
